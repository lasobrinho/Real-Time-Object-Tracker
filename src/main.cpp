...
void adaptativeBackground(Mat *diff, Mat *bg, vector<vector<int> > *pixelSumGray, int *frameCounter) {
	int rows = diff->rows;
	int cols = diff->cols;
	...
	for (int i = 0; i < rows; i++) {
		for (int j = 0; j < cols; j++) {
			pixelSumGray->at(i).at(j) = pixelSumGray->at(i).at(j) + diff->at<uchar>(i, j);
			avg = (float) pixelSumGray->at(i).at(j) / *frameCounter;
			bg->at<uchar>(i, j) = avg;
		}
	}
}
...
int main(int argc, char** argv) {
	...
	for (;;) {
		...
		applyBlur(&frameGray, 2);

		adaptativeBackground(&frameGray, &background, &pixelSumGray, &frameCounter);
